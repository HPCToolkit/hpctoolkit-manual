...
Without hardware support for associating memory latency directly with individual memory accesses, HPCToolkit approximately attributes the exposed memory latency in a basic block by dividing it up among all of the memory accesses in the block. SIMD analysis involves measuring 4 different metrics - number of total SIMD lanes, number of active SIMD lanes, number of wasted SIMD lanes and SIMD lanes lost due to scalar instructions.
\end{comment}
